    ...
    public Observable<?> apply(@NonNull Observable<? extends Throwable> observable) throws Exception {
        return observable.zipWith(Observable.range(1, count + 1), new BiFunction<Throwable, Integer, Wrapper>() {
            ...
            public ObservableSource<?> apply(@NonNull Wrapper wrapper) throws Exception {
                if (wrapper.index > 1)
                    LogUtils.INSTANCE.d("重试次数：" + (wrapper.index));
                int errCode = 0;
                ...
                if ((wrapper.throwable instanceof ConnectException
                        ...
                        || errCode == ApiException.IErrorCode.ERROR_NONETWORKACTIVITY_EXCEPTION
                        || errCode == ApiException.IErrorCode.ERROR_CONNECTIONTIMEOUT_EXCEPTION
                        || wrapper.throwable instanceof SocketTimeoutException
