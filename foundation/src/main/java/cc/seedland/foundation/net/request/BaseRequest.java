...
import cc.seedland.foundation.net.ApiService;
import cc.seedland.foundation.net.CacheMode;
import cc.seedland.foundation.net.HttpFactory;
import cc.seedland.foundation.net.HttpHeaders;
import cc.seedland.foundation.net.HttpParams;
...
import cc.seedland.foundation.net.interceptor.BaseDynamicInterceptor;
import cc.seedland.foundation.net.interceptor.CacheInterceptor;
import cc.seedland.foundation.net.interceptor.CacheInterceptorOffline;
import cc.seedland.foundation.net.interceptor.HeadersInterceptor;
import cc.seedland.foundation.net.interceptor.NoCacheInterceptor;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    protected HttpFactory.SSLParams sslParams;
    ...
    public BaseRequest(String url) {
        this.url = url;
        context = NetTask.getContext();
        NetTask config = NetTask.getInstance();
    ...
    public R cacheTime(long cacheTime) {
        if (cacheTime <= -1) cacheTime = NetTask.DEFAULT_CACHE_NEVER_EXPIRE;
    ...
    public R addInterceptor(Interceptor interceptor) {
        interceptors.add(CommonUtils.checkNotNull(interceptor, "interceptor == null"));
        return (R) this;
    }

    public R addNetworkInterceptor(Interceptor interceptor) {
        networkInterceptors.add(CommonUtils.checkNotNull(interceptor, "interceptor == null"));
        return (R) this;
    }
    ...
    public R cacheDiskConverter(IDiskConverter converter) {
        this.diskConverter = CommonUtils.checkNotNull(converter, "converter == null");
        return (R) this;
    }
    ...
    public R certificates(InputStream... certificates) {
        this.sslParams = HttpFactory.getSslSocketFactory(null, null, certificates);
        return (R) this;
    }
    ...
    public R certificates(InputStream bksFile, String password, InputStream... certificates) {
        this.sslParams = HttpFactory.getSslSocketFactory(bksFile, password, certificates);
        return (R) this;
    }
    ...
    public void removeCache(String key) {
        NetTask.getRxCache().remove(key).compose(RxUtils.<Boolean>io_main())
                .subscribe(new Consumer<Boolean>() {
                    @Override
                    public void accept(@NonNull Boolean aBoolean) throws Exception {
                        LogUtils.INSTANCE.d("removeCache success!!!");
                    }
                }, new Consumer<Throwable>() {
                    @Override
                    public void accept(@NonNull Throwable throwable) throws Exception {
                        LogUtils.INSTANCE.d("removeCache err!!!"+throwable);
                    }
                });
    }
    ...
    private OkHttpClient.Builder generateOkClient() {
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && sslParams == null
            ...
            OkHttpClient.Builder builder = NetTask.getOkBuilder();
            ...
            final OkHttpClient.Builder newClientBuilder = NetTask.getOkHttpClient().newBuilder();
            if (readTimeOut > 0)
            ...
            if (cookies.size() > 0) NetTask.getCookieJar().addCookies(cookies);
    ...
    private Retrofit.Builder generateRetrofit() {
        if (converterFactories.isEmpty() && adapterFactories.isEmpty()) {
            return NetTask.getRfBuilder().baseUrl(baseUrl);
        } else {
            ...
            if (!converterFactories.isEmpty()) {
                ...
                //获取全局的对象重新设置
                List<Converter.Factory> listConverterFactory = NetTask.getRetrofit().converterFactories();
            ...
            if (!adapterFactories.isEmpty()) {
                ...
                //获取全局的对象重新设置
                List<CallAdapter.Factory> listAdapterFactory = NetTask.getRetrofit().callAdapterFactories();
    ...
    private RxCache.Builder generateRxCache() {
        final RxCache.Builder rxCacheBuilder = NetTask.getRcBuilder();
        switch (cacheMode) {
            case NO_CACHE://不使用缓存
            ...
            case DEFAULT://使用Okhttp的缓存
                if (this.cache == null) {
                    File cacheDirectory = NetTask.getCacheDirectory();
                    if (cacheDirectory == null) {
                        cacheDirectory = new File(NetTask.getContext().getCacheDir(), "okhttp-cache");
                    } else {
                        if (cacheDirectory.isDirectory() && !cacheDirectory.exists()) {
                            cacheDirectory.mkdirs();
                        }
                    }
                    this.cache = new Cache(cacheDirectory, Math.max(5 * 1024 * 1024, NetTask.getCacheMaxSize()));
                }
                String cacheControlValue = String.format("max-age=%d", Math.max(-1, cacheTime));
                final CacheInterceptor REWRITE_CACHE_CONTROL_INTERCEPTOR = new CacheInterceptor(NetTask.getContext(), cacheControlValue);
                final CacheInterceptorOffline REWRITE_CACHE_CONTROL_INTERCEPTOR_OFFLINE = new CacheInterceptorOffline(NetTask.getContext(), cacheControlValue);
            ...
            case CACHEANDREMOTEDISTINCT:
                ...
                if (diskConverter == null) {
                    ...
                    tempRxCacheBuilder.cachekey(CommonUtils.checkNotNull(cacheKey, "cacheKey == null"))
                            .cacheTime(cacheTime);
                    return tempRxCacheBuilder;
                } else {
                    final RxCache.Builder cacheBuilder = NetTask.getRxCache().newBuilder();
                    cacheBuilder.diskConverter(diskConverter)
                            .cachekey(CommonUtils.checkNotNull(cacheKey, "cacheKey == null"))
                            .cacheTime(cacheTime);
                    return cacheBuilder;
                }
        }
